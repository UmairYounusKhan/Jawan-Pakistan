...
const mobilesSelect = document.getElementById('mobiles');
const mobileTypesSelect = document.getElementById('mobileTypes');
...
Object.keys(mobiles).forEach(brand => {
    ...
    mobilesSelect.add(option);
});

mobilesSelect.onchange = function() {
    const selectedBrand = mobilesSelect.value;

    mobileTypesSelect.innerHTML = '<option value="">Select your mobile model</option>';

    if (selectedBrand) {
        Object.keys(mobiles[selectedBrand]).forEach(type => {
            const option = document.createElement('option');
            option.value = type;
            option.text = type;
            mobileTypesSelect.add(option);
        });
    }
...
function searchMobile() {
    const selectedBrand = mobilesSelect.value;
    const selectedModel = mobileTypesSelect.value;

    if (selectedBrand && selectedModel) {
        const mobileDetails = mobiles[selectedBrand][selectedModel];
        let details = `You selected: ${ selectedBrand.charAt(0).toUpperCase() + selectedBrand.slice(1) } - ${selectedModel}<br><br>`;
        
        for (let key in mobileDetails) {
            if (typeof mobileDetails[key] === 'object') {
                details += `<strong>${key}:</strong><br>`;
                for (let subKey in mobileDetails[key]) {
                    details += `${subKey}: ${JSON.stringify(mobileDetails[key][subKey])}<br>`;
                }
            } else {
                details += `<strong>${key}:</strong> ${mobileDetails[key]}<br>`;
            }
        }

        resultDiv.innerHTML = details;
    } else {
        resultDiv.innerHTML = 'Please select both a brand and a model.';
    }
}
